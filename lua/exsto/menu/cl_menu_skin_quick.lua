...
SKIN.PrintName 		= "Exsto Quick Skin"
SKIN.Author 		= "Prefanatic"
SKIN.DermaVersion	= 1
SKIN.GwenTexture	= Material( "gwenskin/exsto_quick.png" )
...
SKIN.Colours.Label.Default				= Color( 59, 59, 59, 255 );
SKIN.Colours.Label.Bright 				= Color( 0, 180, 255, 255 );
--SKIN.Colours.Label.Dark					= GWEN.TextureColor( 4 + 8 * 8, 500 );
SKIN.Colours.Label.Dark					= Color( 51, 68, 73, 255 );
...
function SKIN:PaintPanel( panel, w, h )
	...
	if panel.actAsFrameMat then
		return self.tex.Window.Normal( 0, 0, w, h, panel.m_bgColor );
	end
...
function SKIN:PaintButton( panel, w, h )
	...
	if ( !panel.m_bBackground ) then return end
	if !panel._Col then panel._Col = Color( 255, 255, 255, 255 ) end
...
function SKIN:PaintListViewLine( panel, w, h )
	...
	elseif ( panel.m_bAlt ) then
		...
		return self.tex.Input.ListBox.EvenLine( 0, 0, w, h );
	         
	end
	
	self.tex.Input.ListBox.OddLine( 0, 0, w, h );
...
derma.DefineSkin( "ExstoQuick", "Exsto's default shipping skin", SKIN )